const sampleData = [
  {roll: 'SLTC-101', year: '2025', course:'ADCA', name: 'Arjun', father: 'Sh. Madanpal', mother: 'Smt. Lata',
    subjects: [
  ...
  {roll: 'SLCTC-102', year: '2025', course:'DCA', name: 'Sita Verma', father: 'Sh. Rajesh Verma', mother: 'Smt. Meena Verma',
    subjects: [
  ...
  {roll: 'SLCTC-201', year: '2024', course:'DCA', name: 'Amit Singh', father: 'Sh. Vinod Singh', mother: 'Smt. Lata Singh',
    subjects: [
...
function gradeFromPct(p){ if(p>=85) return 'A+'; if(p>=75) return 'A'; if(p>=60) return 'B'; if(p>=45) return 'C'; return 'F'; }
function statusFromPct(p){ return p >= 45 ? 'Pass' : 'Fail'; }
function divisionFromPct(p){ if(p>=60) return 'First Division'; if(p>=45) return 'Second Division'; return 'Fail'; }
...
function renderResult(rec){
  ...
  const subjectsHtml = rec.subjects.map(s=>`<tr><td>${s.name}</td><td>${s.theory} / ${s.theoryMax}</td><td>${s.practical} / ${s.practicalMax}</td></tr>`).join('');

  // Generate QR code
  const qrCodeDiv = document.createElement('div');
  new QRCode(qrCodeDiv, {text: window.location.href + '?roll=' + rec.roll + '&year=' + rec.year + '&course=' + rec.course, width: 120, height: 120});
  ...
  return `
    <div class="card">
      ...
      <div id="studentQrcode" style="margin-top:12px"></div>
    </div>
  `;
}
...
document.getElementById('searchBtn').addEventListener('click', ()=>{
  ...
  if(!course){ showMessage('Please select a course.'); return; }

  const rec = sampleData.find(r=> r.roll.toLowerCase()===roll.toLowerCase() && r.year===year && r.course===course);
  if(!rec){ showMessage('Result not found for the provided Roll No., Year & Course.'); return; }

  document.getElementById('resultArea').innerHTML = renderResult(rec);
  new QRCode(document.getElementById('studentQrcode'), {
    text: window.location.href + '?roll=' + rec.roll + '&year=' + rec.year + '&course=' + rec.course,
    width: 120,
    height: 120
  });
});
